...
public class CreateImpl implements Create {
    ...
    protected final List<Class<? extends IsEntity>> entityList;
    protected final PojoToDBObject translator;

    @Inject
    public CreateImpl(DB mongoDb, @EntityList List<Class<? extends IsEntity>> entityList, PojoToDBObject translator) {
    ...
    public void doCreate(IsEntity entityToPersist) {
        ...
        for (Class<? extends IsEntity> clazz : entityList) {
            if (clazz.equals(entityToPersist.getClass())) {
                isInList = true;
                break;
            }
        }
